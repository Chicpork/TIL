        ...
        try:
            ...
            if response.status_code == requests.codes.ok:
                ...
                if tree.find('./header/resultCode').text == '00': # normal response
                    self.logger.info("Get success {},{},{},{},{}".format(self.deal_ymd, self.lawd_cd, str(self.num_of_rows), str(self.page_no), response.text[-50:]))
    ...
    for deal_ymd in deal_ymds:
        ...
        for lawd_cd in lawd_cds:
            ...
            if len(lawd_cd_files) == 0:
                ray_queue.put((deal_ymd_path.as_posix(), deal_ymd, lawd_cd, num_of_rows, None, 1))
            else:
                ...
                for page_no in range(1, math.ceil(int(file_infos[2])/int(file_infos[1]))+1):
                    ...
                    if not deal_ymd_path.joinpath(file_name).exists():
                        ray_queue.put((deal_ymd_path.as_posix(), deal_ymd, lawd_cd, num_of_rows, file_infos[2], page_no))


if __name__ == "__main__":
    aptCollector_actors = [aptCollector.remote() for _ in range(num_cpus)]
    ray_queue = queue.Queue()
    ray.put(get_jobLists(ray_queue))

    results = ray.get([aptCollector_actor.run.remote(ray_queue) for aptCollector_actor in aptCollector_actors])