...
class aptCollector(SingletonInstance):
    ...
    def tran_start(self, data):
        self.__set_variables(data)

        while self.total_cnt is None or self.page_no*self.num_of_rows < int(self.total_cnt):
            result_code, data, self.total_cnt = self.__get_apt_trsc_data(self.get_url())
            if result_code == 1:
    ...
    for deal_ymd in deal_ymds:
        ...
        for lawd_cd in lawd_cds:
            ...
            if len(lawd_cd_files) == 0:
                jobLists.append((deal_ymd_path.as_posix(), deal_ymd, lawd_cd, num_of_rows, None, 1))
            else:
                ...
                for page_no in range(1, math.ceil(int(file_infos[2])/int(file_infos[1]))+1):
                    ...
                    if not deal_ymd_path.joinpath(file_name).exists():
                        jobLists.append((deal_ymd_path.as_posix(), deal_ymd, lawd_cd, num_of_rows, file_infos[2], page_no))

    return jobLists

if __name__ == "__main__":
    aptCollector_actors = [aptCollector.remote() for _ in range(num_cpus)]
    jobLists = get_jobLists()
    results = ray.get([aptCollector_actors[ix%num_cpus].tran_start.remote(job) for ix, job in enumerate(jobLists)])
